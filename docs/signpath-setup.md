...
2. 填写 HubSpot 表单：
   - **Email**：`dongfengchat@gmail.com`（你的 SignPath 登录邮箱）
   - **Project name**：`DFCHAT`
   - **Project URL**：`https://github.com/dongfengchat/dfchat`
   - **License**：MIT（或你实际用的协议）
   - **Description**：建议填英文，例如：
     ```
     DFCHAT is an open-source desktop application that combines team chat
     with live streaming in a single self-hosted product. The client is
     built with Electron and React; the server is Go with PostgreSQL and
     SRS for RTMP→HLS transcoding. The project is MIT-licensed and has
     no commercial edition or paid tiers — the entire source tree is
     public on GitHub.

     We distribute a Windows installer via GitHub Releases. Code signing
     would let our users install it without SmartScreen warnings, which
     is the main installation friction reported by early adopters.
     ```
   - **Why open source?**：一两句说明项目愿景，例如：
     ```
     Built as a self-hosted alternative to closed commercial chat suites,
     intended for small teams that want full control over their data.
     ```
3. **提交** → 等基金会邮件
...
| 方案 | 价格 | 时长 | CI 自动签 |
|---|---|---|---|
| ~~**SignPath Trial Plan**~~ | ~~$0（30 天试用）~~ | ~~立即~~ | ❌ **Trial 不支持 Trusted Build Systems**，无法从 GitHub Actions 自动签，**只能 Web UI 手动签** |
| **Azure Trusted Signing** | ~$120/月 | 1–3 天 | ✅ 完整 GitHub Actions 集成 |
| **DigiCert OV** 商业证书 | ~$300/年 | 1–2 周 |
| **DigiCert EV** 商业证书（无 SmartScreen 警告） | ~$700/年 | 2–3 周 + 硬件 token 寄送 |

⚠️ **不要先开 SignPath Trial 试图跑通 CI**——Trial **禁用 Trusted Build Systems**，
GitHub Actions 跑签名会报 `401 "Trusted build system is not allowed"`。
Trial 只适合 Web UI 手动测试。要 CI 自动签必须等 Foundation 通过，或直接付费。
